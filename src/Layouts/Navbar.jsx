      ...
      <div className="bg-[#f0f0f0] text-black shadow-sm">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center text-sm uppercase">
          ...
          <div className="flex items-center gap-4">
            ...
            >
              <span className={`block w-5.5 h-[2.5px] bg-gray-600 rounded transition-transform duration-300 ${isMenuOpen ? 'rotate-45 translate-y-[6px]' : ''}`}></span>
              <span className={`block w-5.5 h-[2.5px] bg-gray-600 my-[3px] rounded transition-opacity duration-300 ${isMenuOpen ? 'opacity-0' : 'opacity-100'}`}></span>
              <span className={`block w-5.5 h-[2.5px] bg-gray-600 rounded transition-transform duration-300 ${isMenuOpen ? '-rotate-45 -translate-y-[6px]' : ''}`}></span>
            </button>
          </div>
        </div>

        {/* Dropdown for Hamburger (Visible on all screens) */}
        {isMenuOpen && (
          <div className="absolute top-full right-0 w-full bg-white shadow-md py-6 px-10 z-40 animate-fadeInDown">
